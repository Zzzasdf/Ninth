...
namespace Ninth
{
    public partial class GameEntry
    {
        public class DownloadCore
        {
            ...
            public DownloadCore(PathConfig pathConfig, JsonCore jsonCore)
            {
            ...
            {
                get => PlayerPrefsConfing.DownloadBundleStartPos;
                set => PlayerPrefsConfing.DownloadBundleStartPos = value;
            }
            ...
            public int GetDownloadBundleStartPos()
            {
                ...
                if (tempVersionConfig.Version != PlayerPrefsConfing.DownloadBundleStartPosFromAssetVersion
                    || versionConfig == null) // 防止出现持久化目录数据被删除但有缓存的情况，这种情况下会少下资源包
                {
                    // 重置版本 与 断点位置
                    PlayerPrefsConfing.DownloadBundleStartPosFromAssetVersion = tempVersionConfig.Version;
                    DownloadBundleStartPos = 0;
                }
                return DownloadBundleStartPos;
            }

            public ProcedureCore.MessageBox MessageBox { get; set; }
            ...
            public VersionConfig GetVersionConfig(string path)
            {
                ...
                {
                    VersionConfig newVersionConfig = jsonCore.ToObject<VersionConfig>(path);
            ...
            public DownloadConfig GetDownloadConfig(string path)
            {
                ...
                {
                    DownloadConfig newDownloadConfig = jsonCore.ToObject<DownloadConfig>(path);

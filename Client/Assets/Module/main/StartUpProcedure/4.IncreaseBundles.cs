        ...
        public async void EnterProcedure()
        {
            ...
            for (int i = 1; i < GameEntry.DownloadCore.IncreaseTypeNodes.Count; i++)
            {
                ...
                for (int index = startIndex; index < endIndex; index++)
                {
                    ...
                    GameEntry.Instance.StartCoroutine(GameEntry.DownloadCore.Download(bundleServerPathList[i - 1](version, bundleName), bundlePersistentDataPathList[i - 1](bundleName)));
                    //if (!result)
                    //{

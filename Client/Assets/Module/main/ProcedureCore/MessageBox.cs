...
namespace Ninth
{
    public partial class GameEntry
    {
        public partial class ProcedureCore
        {
            ...
            {
                private DownloadCore downloadCore { get; }
                public MessageBox(DownloadCore downloadCore)
                {
                ...
                private void Init()
                {
                    ...
                    Dictionary<string, Button> btnDic = node.transform.GetComponentsInChildren<Button>().ToDictionary(value => value.name, value => value);
                    Dictionary<string, Text> txtDic = node.transform.GetComponentsInChildren<Text>().ToDictionary(value => value.name, value => value);
                ...
                public async Task<ProcedureInfo> DownloadBeforce(string strMessage, string strFirst, Func<ProcedureInfo> btnFirstClickedFunc, string strSecond, Func<ProcedureInfo> btnSecondClickedFunc)
                {
                ...
                {
                    txtMessage.text = string.Format(stringFormat, 100.ToString(), downloadCore.GetCompleteDownloadIncreaseBundleAmount, downloadCore.GetAllIncreaseBundleCount);
                    Clear();
                }

                private void RefreshProgress(float deltaTime)
                {
                    long curSize = downloadCore.GetCompleteDownloadIncreaseBundleProgress + (long)(curPackSize * downloadCore.Progress);
                    long totalSize = downloadCore.GetTotalIncreaseBundleSize;
                    if (totalSize == 0)
                    {
                        return;
                    }
                    ...
                    {
                        txtMessage.text = string.Format(stringFormat, curPackPercent.ToString(), downloadCore.GetCompleteDownloadIncreaseBundleAmount, downloadCore.GetAllIncreaseBundleCount);
                    }
                }

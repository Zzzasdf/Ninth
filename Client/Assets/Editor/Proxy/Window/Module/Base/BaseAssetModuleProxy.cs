...
namespace Ninth.Editor
{
        ...
        where TChild: MonoBehaviour
    {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    using (new EditorGUILayout.VerticalScope())
                    {
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            ...
                            stringList.AddRange(baseAssetModuleConfig.P2MAssetReference.Parents.Select(x => $"{x.Asset.name}").ToList());
        ...
        // private void RenderRepeatAppointParent()
        // {
        //     using (new EditorGUILayout.VerticalScope(frameBox))
        //     {
        //         var repeatViewGroups = viewConfig.ViewInfos
        //             .GroupBy(x => x.BaseView)
        //             .Where(x => x.Count() > 1)
        //             .ToList();
        //         foreach (var repeatViewGroup in repeatViewGroups)
        //         {
        //             foreach (var repeatView in repeatViewGroup)
        //             {
        //                 using (new EditorGUILayout.HorizontalScope(frameBox))
        //                 {
        //                     EditorGUILayout.ObjectField(repeatView.BaseView, typeof(BaseView), false);
        //                     if (GUILayout.Button($"Remove[{repeatView.Index}]"))
        //                     {
        //                     }
        //                 }
        //             }

...
namespace Ninth.Editor
{
    ...
    public class UIHelperEditor : UnityEditor.Editor
    {
        ...
        {
            m_Helper = (UIHelper)target;
            if (m_Helper.m_UIHelperBarModes == null)
            {
                m_Helper.m_UIHelperBarModes = new List<UIHelperBarMode>();
            }
            if(m_Helper.m_DataBarModes == null)
            {
                m_Helper.m_DataBarModes = new List<UIHelperBarMode>();
            }
        ...
        private void SetHelperBarMode()
        {
            ...
            UIHelperBarMode helpBarMode = (UIHelperBarMode)EditorGUILayout.EnumFlagsField(m_Helper.m_UIHelperBarMode);

            if (helpBarMode != m_Helper.m_UIHelperBarMode)
            {
                m_Helper.m_UIHelperBarMode = helpBarMode;
                // Check
                for (int index = m_Helper.m_UIHelperBarModes.Count - 1; index >= 0; index--)
                {
                    if (m_Helper.m_Keys[index].Values.Count > 0)
                    {
                        m_Helper.m_UIHelperBarMode |= m_Helper.m_UIHelperBarModes[index];
                ...
                m_Helper.m_UIHelperBarModes.Clear();
                foreach (var item in Enum.GetValues(typeof(UIHelperBarMode)))
                {
                    UIHelperBarMode barMode = (UIHelperBarMode)item;
                    if (m_Helper.m_UIHelperBarMode.HasFlag(barMode))
                    {
                        m_Helper.m_UIHelperBarModes.Add(barMode);
                    }
                }
            }
            if (m_Helper.m_UIHelperBarModeIndex > m_Helper.m_UIHelperBarModes.Count - 1)
            {
                m_Helper.m_UIHelperBarModeIndex = m_Helper.m_UIHelperBarModes.Count - 1;
            }
            if (m_Helper.m_UIHelperBarModes.Count > 0 && m_Helper.m_UIHelperBarModeIndex < 0)
            {
                m_Helper.m_UIHelperBarModeIndex = 0;
            }
        ...
        private void SetScrollView()
        {
            ...
            for (int index = 0; index < m_Helper.m_UIHelperBarModes.Count; index++)
            {
                UIHelperBarMode barMode = m_Helper.m_UIHelperBarModes[index];
                if(m_Helper.m_DataBarModes.Count - 1 < index
                    || m_Helper.m_DataBarModes[index] != m_Helper.m_UIHelperBarModes[index])
                { 
                ...
                barNames.Add(m_Helper.m_UIHelperBarModes[index].ToString() + $"({m_Helper.m_Keys[index].Values.Count})");
            }
            int modeIndex = GUILayout.Toolbar(m_Helper.m_UIHelperBarModeIndex, barNames.ToArray());

            if (modeIndex != m_Helper.m_UIHelperBarModeIndex)
            {
                m_Tip = string.Empty;
                m_Helper.m_UIHelperBarModeIndex = modeIndex;
            }
        ...
        private void SetValue()
        {
            if (m_Lock) GUI.enabled = false;
            if (m_Helper.m_UIHelperBarModes != null
                && m_Helper.m_UIHelperBarModeIndex < m_Helper.m_UIHelperBarModes.Count
                && m_Helper.m_UIHelperBarModes.Count > 0)
            {
                switch(m_Helper.m_UIHelperBarModes[m_Helper.m_UIHelperBarModeIndex])
                {
                    case UIHelperBarMode.Button:
                        {
                            SetDictionary(m_Helper.m_Keys[m_Helper.m_UIHelperBarModeIndex].Values, m_Helper.m_Values[m_Helper.m_UIHelperBarModeIndex].Values, typeof(Button));
                            break;
                        }
                    case UIHelperBarMode.Text:
                        {
                            SetDictionary(m_Helper.m_Keys[m_Helper.m_UIHelperBarModeIndex].Values, m_Helper.m_Values[m_Helper.m_UIHelperBarModeIndex].Values, typeof(Text));
                            break;
                        }
                }
            }
        ...
        private void SetDictionary(List<string> names, List<UnityEngine.Object> objs, Type type)
        {
            ...
            // Add
            int modeIndex = m_Helper.m_UIHelperBarModeIndex;
            if (m_AddKeys == null)
            {
